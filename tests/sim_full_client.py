...
def _profile_params(profile: str) -> tuple[float, float, float]:
    ...
    if profile == "fast":
        return 0.90, 150.0, 100.0
    if profile == "medium":
        return 0.55, 105.0, 72.0
    if profile == "slow":
        return 0.20, 65.0, 40.0
    raise ValueError(f"Unknown profile: {profile}")


def main() -> int:
    ap = argparse.ArgumentParser(
        description="Send INNER + OUTER, wait 10s, then send racing lap with fast/medium/slow profile."
    )
    ...
    print(f"\n[*] Waiting {args.wait_after_boundaries} seconds before racing lap...")
    ...
    racing_pts = _generate_driver_points_from_centerline(
        lat0=float(args.center_lat),
        lon0=float(args.center_lon),
        center_xy=center_xy,
        track_width_m=float(args.track_width_m),
    ...
    print("\n[*] Sending RACING lap...")
    racing_lap_id = _send_lap_points(
        base_url=args.base_url,
        token=token,
        track_name=args.track_id,
        lap_type="racing",
    ...
    print(f"\n[*] Building racing line from lap -> track={args.track_id} kind={args.kind} ...")
    out = _build_racing_line_from_lap(
        args.base_url,
        token,
        args.track_id,
        racing_lap_id,
    ...
    print(f"\n[RESULT] racing_lap_id={racing_lap_id}")
